...
export default function TempGauge({ temperature }) {
  ...
  const chartData = [
  { name: 'Current', value: temperature, color: temperature < 20 ? '#3366ff' 
    : temperature >=20 && temperature <=25 ? '#00cc00'
    : temperature > 25 && temperature <=30 ? '#ffff66'
    : '#ff3333' },

  { name: 'Remaining', value: maxTemp - temperature, color: '#ddd' },
  ];
  ...
  return (
    <PieChart width={300} height={250}>
      ...
      <animated.text
        ...
        fill="#333"
      >
        {animatedValue.to((val) => `${val.toFixed(2)} °C`)}
      </animated.text>
    </PieChart>
  );
}
